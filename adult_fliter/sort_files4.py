...
class FileOrganizer:
    ...
    def move_subdirectories(self, source_dir, output_dir):
        """移动子目录到 output 目录（与 Images/Videos 同级）"""
        for item in os.listdir(source_dir):
            item_path = os.path.join(source_dir, item)
            if os.path.isdir(item_path):  # 如果是子目录
                dest_path = os.path.join(output_dir, item)
                # 处理目录名冲突
                counter = 1
                while os.path.exists(dest_path):
                    dest_path = os.path.join(output_dir, f"{item}_{counter}")
                    counter += 1
    ...
    def organize_files(self, source_dir, output_dir):
        ...
        for filename in os.listdir(source_dir):
            ...
            if os.path.isfile(filepath):  # 确保是文件，而不是目录
