...
def git_commit_if_changed(repo_dir, files_to_add, message, tag_prefix="v"):
    import subprocess
    import datetime
    os.chdir(repo_dir)
    ...
    if result.returncode == 0:
        msgbox.showinfo("無需更新", "沒有可需要的更新")
        print("⚠️ 沒有可需要的更新，跳過 commit")
        return
    ...
    tag = f"{tag_prefix}{datetime.datetime.now().strftime('%Y.%m.%d.%H%M')}"
