...
def is_in_google_backup(path):
    return "ｂ、谷歌備份" in path

def list_and_confirm_delete_from_google_backup(duplicates):
    to_delete = []
    for group in duplicates:
        google_backup_files = [p for p in group if is_in_google_backup(p)]
        to_delete.extend(google_backup_files)

    if not to_delete:
        print("\n📁 沒有任何重複檔案來自『ｂ、谷歌備份』")
        return

    print("\n🗂️ 以下是來自『ｂ、谷歌備份』的重複檔案（預計移至垃圾桶）：\n")
    for idx, path in enumerate(to_delete, 1):
        print(f"{idx}. {path}")

    confirm = input("\n⚠️ 是否要將這些檔案全部移至垃圾桶？(y/N)：").strip().lower()
    if confirm == 'y':
        for path in to_delete:
            try:
                send2trash(path)
                print(f"🗑️ 已移至垃圾桶：{path}")
...
def main():
    ...
    if duplicates:
        ...
        list_and_confirm_delete_from_google_backup(duplicates)
    else:
        print("\n✅ 沒有找到任何重複檔案")
